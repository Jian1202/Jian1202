...
module.exports = function projects(theme, entries, offsetY) {
  ...
  const cards = entries.map((entry, index) => {
    ...
    return group([
      ...
      text(entry.note, { x: x + 18, y: 166, fill: colors.text, 'font-family': fonts.display, 'font-size': 13 }),
      text('github / Jian1202', { x: x + 18, y: 194, fill: colors.text, opacity: 0.6, 'font-family': fonts.mono, 'font-size': 10 }),
    ]);
  });
  ...
  return group([
    ...
    text('项目抽屉', { x: 60, y: 44, fill: colors.primary, 'font-family': fonts.display, 'font-size': 28, 'font-weight': 750 }),
    text('PROJECT NOTES', { x: 800, y: 42, fill: colors.text, opacity: 0.62, 'font-family': fonts.mono, 'font-size': 11, 'text-anchor': 'end' }),
    cards,
